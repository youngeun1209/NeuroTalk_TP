...
class myDataset(Dataset):
    def __init__(self, args, mode, data="./", task = "SpokenEEG", recon="Y_mel"):
        ...
        self.win_size = int(math.ceil(args.win_len * args.sampling_rate / args.hop_length))
        self.audio_size = int(math.ceil(args.win_len * args.sample_rate_STT))
    ...
    def reprocess(self, data, idxs):
        ...
        for i in idxs:
            ...
            if eeg.shape[-1] > self.win_size:
                rand_i = int(np.random.randint(self.win_size,eeg.shape[-1],size=1))
                eeg = eeg[:, rand_i-self.win_size : rand_i]
                mel = mel[:, rand_i-self.win_size : rand_i]
                
                rand_i_aud = int(math.ceil(rand_i / self.win_size * self.audio_size))
                audio = audio[rand_i_aud-self.audio_size : rand_i_aud]
                
            if eeg.shape[-1] < self.win_size:
                eeg = pad_data_2d(eeg, self.win_size)
                mel = pad_data_2d(mel, self.win_size)
                audio = pad_data_1d(audio, self.audio_size)
    ...
    def collate_fn(self, data):
        ...
        tail = idx_arr[idx_arr.shape[-1] - (idx_arr.shape[-1] % self.batch_size) :]
        idx_arr = idx_arr[: idx_arr.shape[-1] - (idx_arr.shape[-1] % self.batch_size)]
        idx_arr = idx_arr.reshape((-1, self.batch_size)).tolist()
        if not self.drop_last and tail.shape[-1] > 0:
            idx_arr += [tail.tolist()]
...
def pad_data_1d(x, length, PAD=0):
    x_padded = np.pad(
        x, (length - x.shape[0], 0), mode="constant", constant_values=PAD
    )
    return x_padded
...
def pad_data_2d(x, max_len):
    ...
    s = x.shape[0]
    x_padded = np.pad(
        x, (max_len - x.shape[-1], 0), mode="constant", constant_values=PAD
    )
